...
def account_lookup(steam_user, api_format='py', request_type='account'):
    ...
    # if model_v2.Users.query.filter_by(user_id=unicode(steamID)).first():
    #     return model_v2.Users.query.filter_by(user_id=unicode(steamID)).first().attributes
    ...
    if request_type == 'friends':
        ...
        for g in user_games_page['response']['games']:
            user_apps_list.append(unicode(g['appid']))
        user_multi_games = model_v2.App.query.filter(model_v2.App.app_id.in_(user_apps_list), model_v2.App.multiplayer == True).all()
        ...
        for x in user_games_page['response']['games']:
            ...
            if app_data['missing'] == True:
                app_data = {'missing':True,'app_id':unicode(x['appid']),'minutes_played':x['playtime_forever']}
...
def get_app_data(pid):
    pid = unicode(pid)
    print pid
    query = model_v2.App.query.filter_by(app_id=pid).first()
    # fix for unicode values not coalescing well after new db update script introduced new rows with values containing unsupported characters
    try:
        if query:
...
def get_app_genres(pid):
    pid = unicode(pid)
    genres = []
    for g in model_v2.Genre_App_Map.query.filter_by(apps=pid).all():
...
def get_app_developers(pid):
    pid = unicode(pid)
    developers = []
    # This is an ugly hotfix for not supporting unicode properly with python 2 - throw out developers/publishers (below) if one has an unsupported character (this is unneeded in the python 3 version of the app)
    try:
        for d in model_v2.Developer_App_Map.query.filter_by(apps=pid).all():
...
def get_app_publishers(pid):
    ...
    publishers = []
    try:
        for p in model_v2.Publisher_App_Map.query.filter_by(apps=pid).all():
...
def get_app_languages(pid):
    pid = unicode(pid)
    languages = []
    for l in model_v2.Language_App_Map.query.filter_by(apps=pid).all():
